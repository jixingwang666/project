...
function createApp(options = {}) {
  ...
  // Mock 引擎配置
  const simEngineBaseUrl = (options.simEngineBaseUrl || process.env.SIM_ENGINE_BASE_URL || '').replace(/\/+$/, '');
  const simEnginePath = options.simEnginePath || process.env.SIM_ENGINE_PATH || '/mock/backend/run';
  const externalTimeoutMs = Number(options.externalTimeoutMs || process.env.EXTERNAL_HTTP_TIMEOUT_MS || 15000);

  async function triggerMockSimulation(simulationId, initConfig, mockOptions) {
    if (!simEngineBaseUrl) return null;
    const body = JSON.stringify({
      simulationId,
      initConfig,
      options: {
        fps: Number(mockOptions && mockOptions.fps) || 2,
        totalFrames: Number(mockOptions && mockOptions.totalFrames) || 30,
        randomMode: !!(mockOptions && mockOptions.randomMode)
      }
    });
    const ctrl = new AbortController();
    const timeout = setTimeout(() => ctrl.abort(), externalTimeoutMs);
    try {
      const resp = await fetch(`${simEngineBaseUrl}${simEnginePath}`, {
        method: 'POST',
      ...
      console.log(`[Integration] Mock engine triggered: simulationId=${simulationId}, status=${result.status}`);
      return result;
    } catch (err) {
      console.warn(`[Integration] Mock engine call failed (non-fatal): ${err.message}`);
      return null;
    } finally {
  ...
  app.post('/api/simulations', asyncRoute(async (req, res) => {
    ...
    return withIdempotency(req, res, async () => {
      ...
      // 创建仿真后自动触发 Mock 引擎（非阻塞）
      const mockOptions = req.body.mockOptions || {};
      triggerMockSimulation(simulationId, req.body, mockOptions).catch(() => {});
      return {
        status: 200,
        body: ackEnvelope(req, simulationId, {
          simulationId,
          mockTriggered: !!simEngineBaseUrl
        })
      };
    });
  }));
  ...
  // 手动触发 Mock 引擎
  app.post('/api/integration/backend/start', asyncRoute(async (req, res) => {
    const { simulationId, initConfig, options } = req.body || {};
    if (!simulationId) throw new HttpError(400, 'BAD_REQUEST', 'simulationId is required');
    if (!initConfig) throw new HttpError(400, 'BAD_REQUEST', 'initConfig is required');

    const result = await triggerMockSimulation(simulationId, initConfig, options);
    res.status(200).json(ackEnvelope(req, simulationId, result || { status: 'triggered' }));
  }));
  ...
  // 批量查询：一次性返回所有帧（用于历史页快速加载）
  app.get('/api/simulations/:simId/frames/all', asyncRoute(async (req, res) => {
    const simulationId = req.params.simId;
    const frames = await simulationRepo.getAllFrames(simulationId, {
    ...
    res.status(200).json(
      createEnvelope({
        ...
        requestId: req.res.locals.requestId,
        simulationId,
        messageType: 'FrameList',
        payload: { frames, count: frames.length }
      })
    );
  }));
