...
class SimulationRepository {
  ...
  async getAllFrames(simulationId, options = {}) {
    const limit = Number(options.limit || 200);
    // 使用 query 而非 execute，避免 prepared statement 下 LIMIT 参数化兼容问题
    const sql = `
      SELECT frame_snapshot
      FROM simulation_frames
      WHERE sim_id = ?
      ORDER BY sim_time ASC, frame_index ASC
      LIMIT ${Math.floor(limit)}
    `;

    const [rows] = await this.pool.query(sql, [simulationId]);
