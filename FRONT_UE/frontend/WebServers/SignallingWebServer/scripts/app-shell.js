(function (global) {
  ...
  function bindEvents() {
    ...
    $("btn-history").addEventListener("click", function () {
      ...
      $("btn-history").classList.add("active");
    });

    $("close-history").addEventListener("click", function () {
      $("history-modal").classList.remove("show");
      $("btn-history").classList.remove("active");
    });

    $("refresh-history").addEventListener("click", function () {
    ...
    // --- 灾害控制按钮（→ Socket.IO → 中台 → UE） ---

    $("btn-fire").addEventListener("click", function () {
      try {
        global.MiddlewareClient.emitTriggerFire();
        addLog("发送火灾指令", "info");
        setAck("pending", "火灾指令已发送");
      } catch (err) {
        addLog("火灾指令失败: " + err.message, "error");
        setAck("error", "指令发送失败");
      }
    });
    ...
    $("btn-water").addEventListener("click", function () {
      try {
        global.MiddlewareClient.emitTriggerWater();
        addLog("发送水灾指令", "info");
        setAck("pending", "水灾指令已发送");
      } catch (err) {
        addLog("水灾指令失败: " + err.message, "error");
        setAck("error", "指令发送失败");
      }
    });

    $("btn-both").addEventListener("click", function () {
      try {
        global.MiddlewareClient.emitTriggerBoth();
        addLog("发送水火同发指令", "info");
        setAck("pending", "水火指令已发送");
      } catch (err) {
        addLog("水火指令失败: " + err.message, "error");
        setAck("error", "指令发送失败");
      }
    });
    ...
    $("btn-stop-all").addEventListener("click", function () {
      try {
        global.MiddlewareClient.emitStopAll();
        addLog("发送停止指令", "info");
        setAck("pending", "停止指令已发送");
      } catch (err) {
        addLog("停止指令失败: " + err.message, "error");
        setAck("error", "指令发送失败");
      }
    });
